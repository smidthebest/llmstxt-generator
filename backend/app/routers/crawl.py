    ...
    if job.status in ("completed", "failed"):
        ...
        async def finished_stream():
            for p in stored_pages:
                yield f"data: {json.dumps({'type': 'page_crawled', 'url': p.url, 'title': p.title, 'description': p.description, 'category': p.category, 'relevance_score': round(p.relevance_score, 2), 'depth': p.depth})}\n\n"
            yield f"data: {json.dumps({'type': 'progress', 'pages_found': job.pages_found, 'pages_crawled': job.pages_crawled, 'pages_changed': job.pages_changed, 'max_pages': job.pages_crawled})}\n\n"
            if job.status == "completed":
                yield f"data: {json.dumps({'type': 'completed'})}\n\n"
            else:
                yield f"data: {json.dumps({'type': 'failed', 'error': job.error_message})}\n\n"
